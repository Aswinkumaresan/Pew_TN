...
export function DSRNIACrimeEntryForm({ onClose }: { onClose: () => void }) {
  const [activeStep, setActiveStep] = useState(0);
  const [accusedData, setAccusedData] =
    useState<AccusedData>(defaultAccusedData);

  // Case details
  const [caseId] = useState("DSR-NIA-" + Math.random().toString(36).substring(2, 10).toUpperCase());
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">NIA/Crime Case Entry</p>
        </div>
      </div>
      ...
      {activeStep === 1 && (
        <div className="space-y-5">
          <h3 className="text-sm font-bold text-foreground">NIA/Crime Seizure Details</h3>
